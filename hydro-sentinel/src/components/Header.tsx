...
import { Waves, Bell, User, CheckCircle2, ShieldAlert, Play, Pause } from 'lucide-react';
  ...
  return (
    <header className="flex justify-between items-center w-full px-6 md:px-8 h-16 sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-outline-variant/30 shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        {dashboardView === 'alarm' && (
          <span className="hidden sm:inline bg-red-500/15 border border-red-500/30 px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider text-red-400 font-mono animate-pulse">
            ALARM ACTIVE
          </span>
        )}
      </div>
        ...
        <div className="hidden sm:flex items-center">
          {isSimulating ? (
            <button
              ...
              className="cursor-pointer px-3 py-1.5 bg-red-500/20 text-red-300 border border-red-500/30 rounded-lg text-[10px] font-bold font-sans uppercase tracking-wider flex items-center gap-1.5 hover:bg-red-500/30 transition-all"
            >
            ...
            <button
              ...
              className="cursor-pointer px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white border border-emerald-500/40 rounded-lg text-[10px] font-bold font-sans uppercase tracking-wider flex items-center gap-1.5 transition-all"
            >

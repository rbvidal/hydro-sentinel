...
interface SidebarProps {
  ...
  dashboardView: 'normal' | 'alarm' | 'telemetry' | 'settings';
  onSetDashboardView: (view: 'normal' | 'alarm' | 'telemetry' | 'settings') => void;
}
  ...
  const menuItems = [
    {
      ...
      icon: Activity,
      active: currentTab === 'live' && dashboardView !== 'telemetry' && dashboardView !== 'settings',
      onClick: () => {
        onChangeTab('live');
        if (dashboardView === 'telemetry' || dashboardView === 'settings') {
          onSetDashboardView('normal');
        }
      },
    },
    {
      ...
      name: 'Telemetry',
      icon: Cpu,
      active: dashboardView === 'telemetry',
      onClick: () => {
        onChangeTab('live');
        onSetDashboardView('telemetry');
      },
    },
    {
      id: 'history',
      ...
      active: currentTab === 'history' && dashboardView !== 'telemetry',
      onClick: () => {
        onChangeTab('history');
        if (dashboardView === 'telemetry' || dashboardView === 'settings') onSetDashboardView('normal');
      },
    },
    {
      id: 'sensors',
      name: 'Sensor Health',
      icon: Radio,
      active: currentTab === 'sensors' && dashboardView !== 'telemetry',
      onClick: () => {
        onChangeTab('sensors');
        if (dashboardView === 'telemetry' || dashboardView === 'settings') onSetDashboardView('normal');
      },
